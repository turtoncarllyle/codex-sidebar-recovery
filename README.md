...
这个技能将一次已验证的 Windows 故障恢复经验整理为可复用流程：先分清显示设置与缓存损坏，再从现存数据库记录重建侧栏元数据。它不是会话清理工具，不删除会话，不修改数据库，也不创建新的项目源码目录。

> 这是社区维护的恢复工具，不是 OpenAI 官方修复程序。实际恢复支持 Windows；内部存储格式不是稳定 API，必须先预演。仅在数据库记录仍完整、当前全局配置为可解析 JSON 时适用。
...
- Windows 独立等待程序，当前助手退出后仍可继续；不强制结束应用。
- 使用虚构数据的自动测试，以及 Windows / Ubuntu 测试工作流。Ubuntu 仅验证规划逻辑和模拟恢复，不代表支持 Linux 客户端恢复。
...
本机脚本需要 Python 3.10+。优先使用 Codex 已提供的 Python 运行时，不要求修改系统 PATH；独立等待还需要其同目录的 `pythonw.exe`、PowerShell 5.1+ 与可用的 WMI/CIM。没有本机文件与进程访问权限的云端助手只能提供离线操作指导。
...
4. 确认范围后，应用已退出可直接恢复；应用仍在运行则启动独立等待程序，核对其父进程及 `waiting_for_exit` 状态后再自然退出。
...
实际恢复、取消等待和回滚说明见[退出后恢复操作](codex-sidebar-recovery/references/windows-handoff.md)。

## 这次经验的关键点

**数据库有项目，不代表侧栏缓存也有项目。** 已观察到的故障中，项目表和会话外键仍完整，但桌面配置只剩一个项目；配置重置的具体触发进程没有得到证明，不能一概归因于某个工具。

**不要让仍在运行的应用覆盖修复。** 该案例对应版本会把内存状态写回全局 JSON。普通子进程可能随 Codex 退出；后台窗口隐藏也不代表进程独立。必须检查等待程序真正由 WMI 托管，再让用户退出。
...
- 不适用于 SQLite 本身丢失/损坏、未知字段结构、普通 ChatGPT 云端项目或 macOS 自动写入。当前 JSON 已损坏时先做人工取证和备份，不用空对象替代。
...
测试不接触真实 Codex 配置。进程检查在恢复测试中被模拟；Windows 专用测试另行验证真实进程枚举和锁互斥。它们不能证明所有客户端版本兼容，真实恢复后仍需检查界面。
