...
This skill turns a verified Windows recovery into a reusable workflow: distinguish a display preference from damaged sidebar metadata, then rebuild only that metadata from existing database records. It is not a session-cleanup tool: it does not delete threads, write to the database, or create source-code directories.

> This is a community recovery tool, not an official OpenAI repair utility. Applying recovery is Windows-only. Internal storage is not a stable API; preview first. The database must remain intact and the current global state must be valid JSON.
...
- An independent Windows waiter that can survive the current assistant exiting, without forcibly stopping the app.
- Synthetic-data tests and a Windows / Ubuntu test workflow. Ubuntu tests cover planning and simulated recovery, not Linux desktop repair support.
...
Local scripts require Python 3.10+. Prefer an available Codex Python runtime without changing system PATH. The detached waiter also requires its sibling `pythonw.exe`, PowerShell 5.1+, and working WMI/CIM. A cloud-only assistant without local file/process access can provide instructions, but cannot repair your local app directly.
...
4. After authorization, apply while the app is closed, or start the independent waiter and verify its parent and `waiting_for_exit` status before exiting naturally.
...
See the [Windows handoff guide](codex-sidebar-recovery/references/windows-handoff.md) for applying, cancellation, and rollback. Detailed references are Chinese; commands and status identifiers remain in English.
...
**Do not race the running app.** The affected build persisted in-memory global state back to JSON. A normal child process may also exit with Codex; hiding a console window does not establish independence. Verify that WMI owns the waiter before asking the user to quit.
...
- Missing/corrupt databases, unknown storage formats, ordinary ChatGPT cloud projects, and automatic macOS writes are outside scope. If current JSON is corrupt, preserve evidence for manual recovery rather than substituting an empty object.
...
Tests never access real Codex state. Recovery tests mock process checks; Windows-specific tests separately exercise native enumeration and lock exclusion. They do not prove compatibility with every desktop build. Verify the UI after any real recovery.
